 ...
 * @Date: 2024/9/4 13:46
 */
class CalendarDrawer(context: Context,delegate: CalendarViewDelegate) : BaseCalendarDrawer(context, delegate) {
    ...
    init {
        ...
        val paint = delegate.switchTextPaint(BaseView.PAINT_CURRENT_MONTH_DATE)
    ...
    override fun onDrawText(
        ...
        if (isSelected) {
            val paint = delegate.switchTextPaint(BaseView.PAINT_CURRENT_SELECT_DATE)
            canvas.drawText(calendar.day.toString(), cx.toFloat(), mTextBaseLine + top, paint)
            delegate.switchTextPaint(BaseView.PAINT_SELECT_LUNAR_DATE)
            canvas.drawText(
                calendar.lunar,
                cx.toFloat(),
