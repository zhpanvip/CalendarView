...
public final class CalendarUtil {
     ...
     */
    public static boolean isWeekend(CalendarDay calendar) {
     ...
     * @return 不需要多余行的高度
     */
    public static int getOriginMonthViewHeight(int year, int month, int itemHeight, int weekStartWith) {
        ...
        return (preDiff + monthDaysCount + nextDiff) / 7 * itemHeight - 3;
    }
     ...
     * @return
     */
    public static int getMonthViewItemCount(int year, int month, int weekStartWith) {
        int preDiff = getMonthViewStartDiff(year, month, weekStartWith);
        int monthDaysCount = getMonthDaysCount(year, month);
        int nextDiff = getMonthEndDiff(year, month, monthDaysCount, weekStartWith);
     ...
     * @return 不需要多余行的高度
     */
    public static int getMonthViewHeight(int year, int month, CalendarViewDelegate delegate) {
        int mode = delegate.getMonthViewShowMode();
        int paddingTop = delegate.getCalendarPaddingTop();
        if (mode == CalendarViewDelegate.MODE_ALL_MONTH) {
            return delegate.getCalendarItemHeight() * 6 + paddingTop;
        }
        int weekStartWith = delegate.getWeekStart();
        int monthViewItemCount = getMonthViewItemCount(year, month, weekStartWith);
        int itemHeight = delegate.getCalendarItemHeight();
        int monthViewOriginHeight = monthViewItemCount * itemHeight;
        return monthViewOriginHeight + delegate.getMonthViewHeightOffset(monthViewOriginHeight) + paddingTop;
    }
     ...
     */
    public static int getWeekFromDayInMonth(CalendarDay calendar, int weekStart) {
     ...
     * @return 获取上一个日子
     */
    public static CalendarDay getPreCalendar(CalendarDay calendar) {
        java.util.Calendar date = java.util.Calendar.getInstance();
        ...
        CalendarDay preCalendar = CalendarDay.obtain();
    ...
    public static CalendarDay getNextCalendar(CalendarDay calendar) {
        java.util.Calendar date = java.util.Calendar.getInstance();
        ...
        CalendarDay nextCalendar = CalendarDay.obtain();
     ...
     */
    static int getMonthViewStartDiff(CalendarDay calendar, int weekStart) {
        java.util.Calendar date = java.util.Calendar.getInstance();
        date.set(calendar.getYear(), calendar.getMonth() - 1, 1, 12, 0, 0);
     ...
     * @return 返回某个日期是星期几
     */
    static int getWeekFormCalendar(CalendarDay calendar) {
        java.util.Calendar date = java.util.Calendar.getInstance();
     ...
     */
    static int getWeekViewIndexFromCalendar(CalendarDay calendar, int weekStart) {
     ...
     * @return 是否在日期范围內
     */
    static boolean isCalendarInRange(CalendarDay calendar,
     ...
     */
    public static int getWeekFromCalendarStartWithMinCalendar(CalendarDay calendar,
            int minYear, int minYearMonth, int minYearDay,
            int weekStart) {
        java.util.Calendar date = java.util.Calendar.getInstance();
     ...
     * @return 该星期的第一天日期
     */
    public static CalendarDay getFirstCalendarStartWithMinCalendar(int minYear, int minYearMonth, int minYearDay, int week, int weekStart) {
        java.util.Calendar date = java.util.Calendar.getInstance();
     ...
     * @return 是否在日期范围内
     */
    public static boolean isCalendarInRange(CalendarDay calendar, CalendarViewDelegate delegate) {
     ...
     */
    public static int differ(CalendarDay calendar1, CalendarDay calendar2) {
        if (calendar1 == null) {
    ...
    public static int compareTo(int minYear, int minYearMonth, int minYearDay,
            int maxYear, int maxYearMonth, int maxYearDay) {
        CalendarDay first = CalendarDay.obtain();
        first.setYear(minYear);
        first.setMonth(minYearMonth);
        first.setDay(minYearDay);

        CalendarDay second = CalendarDay.obtain();
     ...
     * @return 为月视图初始化日历项
     */
    static List<CalendarDay> initCalendarForMonthView(int year, int month, CalendarDay currentDate, int weekStar) {
        java.util.Calendar date = java.util.Calendar.getInstance();
        ...
        List<CalendarDay> mItems = new ArrayList<>();
        ...
        for (int i = 0; i < size; i++) {
            CalendarDay calendarDate = CalendarDay.obtain();
            if (i < mPreDiff) {
    ...
    static List<CalendarDay> getWeekCalendars(CalendarDay calendar, CalendarViewDelegate mDelegate) {
    ...
    static List<CalendarDay> initCalendarForWeekView(CalendarDay calendar, CalendarViewDelegate mDelegate, int weekStart) {
        ...
        CalendarDay selectCalendar = CalendarDay.obtain();
     ...
     */
    static CalendarDay getFirstCalendarFromMonthViewPager(int position, CalendarViewDelegate delegate) {
        CalendarDay calendar = CalendarDay.obtain();
        ...
        if (delegate.getDefaultCalendarSelectDay() != CalendarViewDelegate.FIRST_DAY_OF_MONTH) {
            ...
            CalendarDay indexCalendar = delegate.mIndexCalendar;
     ...
     * @return 获取边界访问日期
     */
    static CalendarDay getRangeEdgeCalendar(CalendarDay calendar, CalendarViewDelegate delegate) {
        ...
        CalendarDay minRangeCalendar = delegate.getMinRangeCalendar();
     ...
     * @return 是否是最小访问边界了
     */
    private static boolean isMinRangeEdge(CalendarDay calendar, CalendarViewDelegate delegate) {
        java.util.Calendar c = java.util.Calendar.getInstance();
