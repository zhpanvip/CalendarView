...
public abstract class YearView extends View {
     ...
     * 月视图除去 padding 后的宽度
     */
    public static int monthViewWidth;
    /**
     * 月视图中月份 Text 以及 paddingTop部分的高度
     */
    public static int monthViewTopLayoutHeight;

    protected static final int[] sSelectedDayPivot = new int[2];
    ...
    final void measureSize(int width, int height) {
        ...
        monthViewWidth = width;
        monthViewTopLayoutHeight = mDelegate.getYearViewMonthPaddingTop() +
        ...
        mMonthTextBaseLine = mDelegate.getYearViewMonthHeight() / 2f - monthMetrics.descent +
                (monthMetrics.bottom - monthMetrics.top) / 2f;
     ...
     * @param y 日历Card y起点坐标
     */
    protected abstract void onDrawScheme(Canvas canvas, CalendarDay calendar, int x, int y);
